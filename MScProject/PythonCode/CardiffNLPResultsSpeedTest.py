...
# function to preprocess text to remove any unwanted special characters
def preprocess(text):
    return ''.join(e for e in text if e.isalnum() or  e.isspace())

def computeSentiment(text):
    #deletes generated directory - must be deleted before each model run
    path = Path("cardiffnlp")
    ...
    #run model and put in usable format
    output = model(**encoded_input)
    scores = output[0][0].detach().numpy()
    scores = softmax(scores)

    # the sentiment score is output as an array of negative probability, followed by a neutral probability
    # and a positive probability (which sums to 1). In order to evaluate this on a scale of [-1, 1],
    # minus negative probability from the positive.
...
#--------------------------------------------------
#get current time in milliseconds, so that time at end of code run can be compared
...
#this code to initialise the sentiment analysis model is taken from cardiffnlp on hugging face
...
#declaring a list for all times to go into
allTimes = []

#declaring a for loop here for n=20 repeats for the predictions
for i in range (0,20):
    ...
    print("Time to generate results on run ", i+1, " is ", time_diff, "ms")
    allTimes.append(time_diff)

#taking sum of entire list
totalTimeTaken = sum(allTimes)

#average is total divided by length of list
averageTime = round(totalTimeTaken/(len(allTimes) + 1), 1)
...
for i in range(0, len(allTimes)):
    standardDeviation = standardDeviation + (allTimes[i] - averageTime)**2

#to complete standard deviation, divide by N and then square root
standardDeviation = round(math.sqrt(standardDeviation/len(allTimes)), 1)

#standard error on mean is standard deviation divided by root of N
standardError = round(standardDeviation/math.sqrt(len(allTimes)), 1)

print("Total time: " , totalTimeTaken, "ms")
print("Average time: ", averageTime, "ms")
print("Standard deviation: ", standardDeviation, "ms")
print("Standard error on mean: ", standardError, "ms")